...
struct OneBand : Module {
	...
	OneBand() { 
		...
		configParam(EQFREQ_PARAM, .459435f, 10.f, 3.459432f, "Freq", " Hz", 2.f, 20.f);
		configParam(EQBANDWIDTH_PARAM, .1f, 40.f, 15.f, "Q Factor");
		configParam(OUTGAIN_PARAM, 0.f, 7.f, 2.f, "Output Gain", "dB", -10, 20, -13.f);
		//Switch---
	...
	void process(const ProcessArgs &args) override {
		...
		float inL = 0.0f, inR = 0.0f, outLdBVU = 0.0f, outRdBVU = 0.0f,
			outL = simd::clamp(outputs[OUTL_OUTPUT].getVoltage(), -9.9f, 9.9f), outR = simd::clamp(outputs[OUTR_OUTPUT].getVoltage(), -9.9f, 9.9f),
			Gain = params[OUTGAIN_PARAM].getValue() / 2.0f;
		double eqGain = params[EQGAIN_PARAM].getValue() * 5.0 + (simd::clamp(inputs[GAINMOD_INPUT].getVoltage(), -6.5f, 6.5f) * 5.0),
			modInput, eqFreq,
			eqQ = simd::clamp(params[EQBANDWIDTH_PARAM].getValue(), .1f, 40.f) + (4.f * simd::clamp(inputs[BWMOD_INPUT].getVoltage(), .1f, 40.f));
		double sampRate = args.sampleRate, biquadFreq = 3.459432, biquadQ = 15., biquadGain = 0.;
		...
		if (vuDivider.process()) {
			if (params[PREPOST_PARAM].getValue() == 0.0f && !inputs[INR_INPUT].isConnected()) {//pre L/mon
				volUnitIndicatorPEAK.process(args.sampleTime * vuDivider.getDivision(), inL / 10.f);
			} else if (params[PREPOST_PARAM].getValue() == 1.0f && !inputs[INR_INPUT].isConnected()) {//post L/mon
				volUnitIndicatorPEAK.process(args.sampleTime * vuDivider.getDivision(), outLdBVU / 10.f);
			}
			if (params[PREPOST_PARAM].getValue() == 0.0f && inputs[INR_INPUT].isConnected()) {//pre L/R
				volUnitIndicatorPEAK.process(args.sampleTime * vuDivider.getDivision(), (inL / 10.f) + (inR / 10.f) / 2.f);
			} else if (params[PREPOST_PARAM].getValue() == 1.0f && inputs[INR_INPUT].isConnected()) {//post L/R
		...
		if (inputs[FREQMOD_INPUT].isConnected()) {
			...
			modInput = simd::fmin(simd::clamp((inputs[FREQMOD_INPUT].getVoltage() / 5.f), simd::fabs(.01375f), simd::fabs(10.f)), simd::fmax(simd::fabs(.01375f), simd::fabs(10.f)));
			lpf24.setCutoff(110.f);
			lpf24.process(modInput, sampRate);
			eqFreq = simd::clamp(params[EQFREQ_PARAM].getValue(), .11f, .11f) + ((5.f * lpf24.lowpass) * 2);
		} else if (!inputs[FREQMOD_INPUT].isConnected()) {
			lights[FreqParamOn].setBrightness(1); lights[FreqParamOff].setBrightness(0);
			eqFreq = simd::fmax(simd::pow(2.f, params[EQFREQ_PARAM].getValue()), simd::fmin(simd::fabs(1.1726039399558574), simd::fabs(10.)));//min == .01375
		}
		...
		biquadQ = eqQ;
		//Bypass EQ
		params[EQBYPASS_PARAM].getValue() < 1.f ? biquadGain = eqGain :	biquadGain = 0.;
		//set Biquad Values
