...
struct LMH2 : Module {
	...
	LMH2() {
		...
		*/
		...
		configParam(LOW_PARAM, 0.0, M_SQRT2, 1.0, "Low", " dB", -10, 40);
		configParam(MUD_PARAM, 0.0, M_SQRT1_2, 1.0, "Mud", " dB", -10, 40, 6.0206 + 4.7684e-07);
		configParam(HIGH_PARAM, 0.0, M_SQRT2, 1.0, "High", " dB", -10, 40);
		configParam<tpOnOff>(INV_PARAM, 0.f, 1.f, 1.f, "Invert Mud");
		clickFilter.rise = 400.f; // Hz
		clickFilter.fall = 400.f; // Hz
	}
	...
	void process(const ProcessArgs &args) override {
		...
		*/
		...
		if (gLow > 0.f) {
			low->setBiquad(bq_type_lowpass, low_Fc, 0.707, gLow);
			//low->setType(bq_type_lowpass);
			//low->setFc(low_Fc);
			//low->setQ(0.707);
			//low->setPeakGain(gLow);
			//low->calcBiquad();
		}

		if (gHigh > 0.f) {
			high->setBiquad(bq_type_highpass, high_Fc, 0.19597, gHigh);
			//high->setType(bq_type_highpass);
			//high->setFc(high_Fc);
			//high->setQ(0.19597);
			//high->setPeakGain(gHigh);
			//high->calcBiquad();
		}

		if (gMud > 0.f) {
			peak1->setBiquad(bq_type_peak, peak1_Fc, 0.1, 6.9599);
			//peak1->setType(bq_type_peak);
			//peak1->setFc(peak1_Fc);
			//peak1->setQ(0.1);
			//peak1->setPeakGain(6.9599);
			//peak1->calcBiquad();
			peak2->setBiquad(bq_type_peak, peak2_Fc, 0.1, 1.4344);
			//peak2->setType(bq_type_peak);
			//peak2->setFc(peak2_Fc);
			//peak2->setQ(0.1);
			//peak2->setPeakGain(1.4344);
			//peak2->calcBiquad();
			mud->setBiquad(bq_type_lowpass, mud_Fc, 0.71182, gMud);
			//mud->setType(bq_type_lowpass);
			//mud->setFc(mud_Fc);
			//mud->setQ(0.71182);
			//mud->setPeakGain(gMud);
			//mud->calcBiquad();
		}
...
struct LMH2Widget : ModuleWidget {
	LMH2Widget(LMH2 *module) {
		setModule(module);
		setPanel(APP->window->loadSvg(asset::plugin(pluginInstance, "res/BarkLMH.svg")));
		...
		///Ports---
		//Out---
		addOutput(createOutput<BarkOutPort350>(Vec(portX, Y[0]), module, LMH2::OUT_OUTPUT));
		//In---
		addInput(createInput<BarkInPort350>(Vec(portX, Y[1]), module, LMH2::IN_INPUT));
		//Knob---
		addParam(createParam<BarkKnob_26>(Vec(knbX, Y[2]), module, LMH2::LOW_PARAM));
		addParam(createParam<BarkKnob_26>(Vec(knbX, Y[3]), module, LMH2::MUD_PARAM));
		addParam(createParam<BarkKnob_26>(Vec(knbX, Y[4]), module, LMH2::HIGH_PARAM));
		//Switch---
